...
export class RectFit {

  nearLimit = 40;
  //ROTATION_INCREMENT = 5;
  PADDING = 0;

  ROTATION_VALUES = [-0.2, -0.15, -0.1, -.05, -0.02, 0, 0.02, 0.05, 0.1, 0.15, 0.2]; //, 81, 82, 83, 84, 85, 86, 87, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99];

  sin = new Array<number>(this.ROTATION_VALUES.length);
  cos = new Array<number>(this.ROTATION_VALUES.length);

  //sin = new Array<number>(180 / this.ROTATION_INCREMENT);
  //cos = new Array<number>(180 / this.ROTATION_INCREMENT);
  ...
  fitLinesByRotation(imageData: ImageData, threshhold:number = 50) {
    let hlines = new Array<{ y: number, theta: number, highscore: number, yHigh: number }>();
    let vlines = new Array<{ x: number, theta: number, highscore: number, xHigh: number }>();
    const {
      paddedMatrix: binary,
      paddedWidth: width,
      paddedHeight: height
    } = padMatrixSquare(this.asBinarised(imageData), imageData.width, this.PADDING);
    ...
    this.ROTATION_VALUES.forEach(theta => {
      for (let y = 1; y < height - 1; y++) {
        ...
        if (highscore > threshhold) {
          hlines[hlines.length] = {y: y - this.PADDING, theta: theta, highscore, yHigh};
        }
        if (highscoreR > threshhold) {
          vlines[vlines.length] = {x: y - this.PADDING, theta: theta, highscore: highscoreR, xHigh: yHighR};
        }
      }
    });

    //cleanup lines
    vlines=vlines.sort((a, b)=>b.x-a.x).reverse()

    const besthLines = new Array<{ y: number, theta: number, highscore: number }>();
    let best = hlines[0];
    hlines.forEach((value) => {
      if ((isNear(value.yHigh, best.yHigh, this.nearLimit) || isNear(value.y, best.y, this.nearLimit)) && isNear(value.theta, best.theta, 0.1)) {
        if (value.highscore > best.highscore) {
          best = value;
        }
      } else {
        besthLines[besthLines.length] = best;
        best = value;
      }
    });
    const bestvLines = new Array<{ x: number, theta: number, highscore: number }>();
    let bestv = vlines[0];
    vlines.forEach((value) => {
      if ((isNear(value.xHigh, bestv.xHigh, this.nearLimit) || isNear(value.x, bestv.x, this.nearLimit)) && isNear(value.theta, bestv.theta, 0.1)) {
        if (value.highscore > bestv.highscore) {
          bestv = value;
        }
      } else {
        bestvLines[bestvLines.length] = bestv;
        bestv = value;
      }
    });
  ...
  fitPointsByIntersection(
    ...
    bestvLines: { x: number; theta: number; highscore: number }[], width: number, heigt: number) {
    let points = new Array<{ x: number, y: number, score: number }>();

    // For each pair of horizontal and vertical lines, generate a rectangle
    for (const hLine of besthLines) {
      for (const vLine of bestvLines) {

        const x = vLine.x;
        const y = hLine.y;

        const adjX = Math.floor(x + Math.tan(vLine.theta) * y);
        const adjY = y + Math.floor(Math.tan(hLine.theta) * x);

        points.push({x: adjX, y: adjY, score: hLine.highscore * vLine.highscore});
      }
    }
